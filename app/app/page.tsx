...
export default async function Dashboard() {
  ...
  return <div className="grid gap-8">
    ...
    <details className="panel p-5"><summary className="cursor-pointer text-lg font-bold">Archived classrooms & school years</summary><div className="mt-5 grid gap-4">
      ...
      {years.map((year) => <div className="grid gap-3 rounded-xl border border-black/10 p-4 md:grid-cols-[1fr_auto_auto]"><strong>{year}</strong><form action={archiveYear}><input type="hidden" name="schoolYear" value={year} /><button className="btn btn-soft">Archive year</button></form><form action={deleteYear} className="flex gap-2"><input type="hidden" name="schoolYear" value={year} /><input className="field" name="confirmation" placeholder={`DELETE ${year}`} required /><button className="btn bg-red-700">Delete year</button></form></div>)}
    </div></details>
  </div>;
}
