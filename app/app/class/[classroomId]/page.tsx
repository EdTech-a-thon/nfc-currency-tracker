...
export default async function AwardPage({ params }: { params: Promise<{ classroomId: string }> }) {
  ...
  return <div className="grid gap-6">
    ...
    {classroom.archived ? <div className="panel p-8"><h2 className="text-2xl">This classroom is archived</h2><p className="mt-2">Its history remains available, but rewards and purchases are read-only.</p></div> : <OptimisticAward classroomId={classroomId} symbol={classroom.currencySymbol} presets={presets} students={students.map((student) => ({ id: student.id, displayName: student.displayName, balance: student.transactions.reduce((sum, entry) => sum + entry.amount, 0) }))} />}
  </div>;
}
