...
export async function POST(request: Request) {
  ...
  try {
    ...
    const entries = [];
    for (const [index, studentId] of body.studentIds.slice(0, 100).entries()) entries.push(await postEntry({ teacherId: teacher.id, studentId, amount: Number(body.amount), reason: String(body.reason || "Classroom award").slice(0, 200), kind: "AWARD", idempotencyKey: `${key}-${index}` }));
