...
export function BulkCardManager({ cards }: { cards: CardRow[] }) {
  ...
  return <form action={bulkSetCardStatus} className="panel overflow-hidden" onSubmit={(event) => {
    ...
    {selected.length > 0 && <div className="sticky top-28 z-10 grid grid-cols-2 items-center gap-2 border-b-4 border-[#e85d43] bg-[#f9bd72] p-3 shadow-lg sm:top-20 sm:flex sm:flex-wrap"><strong className="col-span-2 mr-auto text-lg sm:col-span-1">{selected.length} card{selected.length === 1 ? "" : "s"} selected</strong><select className="field col-span-2 sm:max-w-48" name="status" required defaultValue=""><option value="" disabled>Bulk action...</option><option value="AVAILABLE">Make available</option><option value="LOST">Mark lost</option><option value="RETIRED">Retire cards</option></select><button className="btn">Apply</button><button className="btn btn-soft" type="button" onClick={() => setSelected([])}>Cancel</button></div>}
    ...
    {visible.map((card) => <label className={`grid min-h-20 cursor-pointer grid-cols-[36px_1fr_auto] items-center gap-3 border-b border-black/10 px-4 py-3 last:border-0 md:min-h-16 md:grid-cols-[48px_90px_110px_130px_1fr] ${selected.includes(card.id) ? "border-l-4 border-l-[#e85d43] bg-[#fff0e8] pl-3 font-bold ring-2 ring-inset ring-[#e85d43]" : ""}`} key={card.id}>
