...
export function BulkCardManager({ cards }: { cards: CardRow[] }) {
  ...
  return <form action={bulkSetCardStatus} className="panel overflow-hidden" onSubmit={(event) => {
    ...
    <div className="grid gap-4 border-b border-black/10 p-5 lg:grid-cols-[1fr_auto_auto] lg:items-end">
      ...
      <label className="label">Show<select className="field min-w-40" value={filter} onChange={(event) => setFilter(event.target.value as typeof filter)}><option value="ALL">All cards ({cards.length})</option>{Object.entries(statusNames).map(([status, label]) => <option value={status} key={status}>{label} ({cards.filter((card) => card.status === status).length})</option>)}</select></label>
      <button className="btn btn-soft" type="button" onClick={toggleVisible}>{allVisibleSelected ? "Clear visible" : "Select visible"}</button>
    </div>
    {selected.map((cardId) => <input type="hidden" name="cardIds" value={cardId} key={cardId} />)}
    {selected.length > 0 && <div className="sticky top-28 z-10 grid grid-cols-2 items-center gap-2 border-b border-black/10 bg-[#f9bd72] p-3 sm:top-20 sm:flex sm:flex-wrap"><strong className="col-span-2 mr-auto sm:col-span-1">{selected.length} selected</strong><select className="field col-span-2 sm:max-w-48" name="status" required defaultValue=""><option value="" disabled>Bulk action...</option><option value="AVAILABLE">Make available</option><option value="LOST">Mark lost</option><option value="RETIRED">Retire cards</option></select><button className="btn">Apply</button><button className="btn btn-soft" type="button" onClick={() => setSelected([])}>Cancel</button></div>}
    ...
    {visible.map((card) => <label className={`grid min-h-20 cursor-pointer grid-cols-[36px_1fr_auto] items-center gap-3 border-b border-black/10 px-4 py-3 last:border-0 md:min-h-16 md:grid-cols-[48px_90px_110px_130px_1fr] ${selected.includes(card.id) ? "bg-[#fff0e8]" : ""}`} key={card.id}>
      <input className="h-5 w-5" type="checkbox" checked={selected.includes(card.id)} onChange={() => setSelected((current) => current.includes(card.id) ? current.filter((id) => id !== card.id) : [...current, card.id])} />
      <strong className="display text-xl">#{card.label}</strong>
      <span className="font-bold tracking-widest">{card.shortCode}</span>
      <span className={`w-fit rounded-full px-3 py-1 text-xs font-bold ${card.status === "ASSIGNED" ? "bg-[#cde7d8]" : card.status === "AVAILABLE" ? "bg-black/10" : card.status === "LOST" ? "bg-amber-200" : "bg-slate-700 text-white"}`}>{statusNames[card.status]}</span>
      <span className="col-span-2 text-sm md:col-span-1">{card.studentName ? <><strong>{card.studentName}</strong>{card.studentClassroom && <span className="text-slate-500"> · {card.studentClassroom}</span>}</> : <span className="text-slate-500">No student</span>}</span>
    </label>)}
