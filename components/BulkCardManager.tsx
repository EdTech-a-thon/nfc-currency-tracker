...
export function BulkCardManager({ cards, onChanged }: { cards: CardRow[]; onChanged: () => void }) {
  ...
  async function applyStatus(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault();
    const status = String(new FormData(event.currentTarget).get("status") ?? "") as CardRow["status"];
    if (!status) return;
    if (!window.confirm(`Change ${selected.length} selected card${selected.length === 1 ? "" : "s"} to ${status.toLowerCase()}? Any current student assignments will end.`)) return;
    setBusy(true);
    try {
  ...
  return <form onSubmit={(event) => void applyStatus(event)} className="panel overflow-hidden">
