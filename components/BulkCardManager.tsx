...
export function BulkCardManager({ cards }: { cards: CardRow[] }) {
  ...
  return <form action={bulkSetCardStatus} className="panel overflow-hidden" onSubmit={(event) => {
    ...
    {selected.length > 0 && <div className="sticky top-20 z-10 flex flex-wrap items-center gap-3 border-b border-black/10 bg-[#f9bd72] p-3"><strong className="mr-auto">{selected.length} selected</strong><select className="field max-w-48" name="status" required defaultValue=""><option value="" disabled>Bulk action...</option><option value="AVAILABLE">Make available</option><option value="LOST">Mark lost</option><option value="RETIRED">Retire cards</option></select><button className="btn">Apply</button><button className="btn btn-soft" type="button" onClick={() => setSelected([])}>Cancel</button></div>}
    ...
    {visible.map((card) => <label className={`grid min-h-16 cursor-pointer grid-cols-[36px_1fr_auto] items-center gap-3 border-b border-black/10 px-4 py-3 last:border-0 md:grid-cols-[48px_90px_110px_130px_1fr] ${selected.includes(card.id) ? "bg-[#fff0e8]" : ""}`} key={card.id}>
