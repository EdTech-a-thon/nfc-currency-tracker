...
export function Scanner({ mode = "award" }: { mode?: "award" | "checkout" }) {
  ...
  return <div className="panel p-4"><div className="grid gap-2 sm:grid-cols-[auto_1fr_auto] sm:items-center"><button className="btn btn-accent" onClick={() => setOpen((value) => !value)}>{open ? "Close camera" : "Scan QR card"}</button><input className="field uppercase" maxLength={40} value={manual} onChange={(event) => setManual(event.target.value)} onKeyDown={(event) => event.key === "Enter" && submit()} placeholder="Enter short code" /><button className="btn" onClick={submit}>Find card</button></div>{error && <p className="mt-3 text-red-700">{error}</p>}{open && <div id="qr-reader" className="mx-auto mt-4 max-w-lg overflow-hidden rounded-xl" />}{student && mode === "award" && <div className="mt-4 rounded-xl bg-[#cde7d8] p-4"><div className="flex items-center justify-between gap-3"><strong className="text-xl">{student.name}</strong><span className="display whitespace-nowrap text-3xl">{student.symbol}{student.balance}</span></div><div className="mt-3 grid grid-cols-2 gap-2 sm:flex sm:flex-wrap">{student.presets.map((preset) => <button className="btn btn-accent" onClick={() => void award(preset.amount, preset.label)} key={preset.label}>+{preset.amount} {preset.label}</button>)}</div></div>}<p className="mt-3 text-sm">{status}</p></div>;
}
